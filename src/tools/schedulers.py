...
__all__ = ["multistep_lr", "cosine_lr", "constant_lr", "get_policy", "exp_lr",  "OneCycle"]
...
def multistep_lr2(args, batch_num):
    """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
    learning_rate = []
    ...
    for epoch in range(args.epochs):
        for _ in range(batch_num):
            if epoch in args.milestones:
                cur_lr = cur_lr * args.lr_gamma
            learning_rate.append(cur_lr)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--base_lr', type = float, default=0.0002, help='none')
    argparser.add_argument('--warmup_lr', type= float, default=1e-6, help='')
    argparser.add_argument('--epochs', type= int, default= 110, help='')
    ...
    args.lr_gamma = 0.333
    args.milestones = [60, 80]
    learning_rate = multistep_lr2(args, 1)
